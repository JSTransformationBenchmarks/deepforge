...
describe('JobLogsAPI', function () {
  ...
  describe('delete', function () {
    var delUrl;
    before(function (done) {
      delUrl = [server.getUrl(), mntPt, 'testProject', 'other', encodeURIComponent('/4/8/l')].join('/');
      superagent.patch(delUrl).send({
        patch: firstLog
      }).end(function (err, res) {
        expect(res.status).equal(200, err);
        done();
    ...
    it('should delete the file from job-logs directory', function (done) {
      superagent.delete(delUrl).end(function (err, res) {
        expect(res.status).equal(204, err);
        superagent.get(delUrl).end(function (err, res) {
          expect(res.status).equal(200, err);
  ...
  function getUrl(project, branch, job) {
    return [server.getUrl(), mntPt, encodeURIComponent(project), encodeURIComponent(branch), encodeURIComponent(job)].join('/');
  }

  function addLog(project, branch, job, log) {
    var deferred = Q.defer();
    console.log('adding log', log);
    superagent.patch(getUrl(project, branch, job)).send({
      patch: log
    }).end(function (err, res) {
      if (err) {
  ...
  describe('migrate', function () {
    var j1 = '/s/p/3',
        ...
        url;
    before(function (done) {
      Q.all([addLog(proj, b1, j1, j1log), addLog(proj, b1, j2, j2log), addLog(proj, b2, j2, 'otherStuff')]).then(() => {
        url = [server.getUrl(), mntPt + '/migrate', encodeURIComponent(proj), encodeURIComponent(b1), encodeURIComponent(b2)].join('/');
    ...
    it('should not crash on bad request', function (done) {
      url = [server.getUrl(), mntPt + '/migrate', encodeURIComponent(proj), encodeURIComponent('someBranch3'), encodeURIComponent(b2)].join('/');
      superagent.post(url).send({
        jobs: [j1]
      }).end(err => done(err));
    });
