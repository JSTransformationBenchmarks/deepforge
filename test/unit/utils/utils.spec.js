...
describe('misc utils', function () {
  ...
  var run = async function (nodePath, filename, result, done) {
    var txt = await fs_readFilePromise(path.join(MODELS_DIR, filename + '.yml'), 'utf8');
  ...
  describe('matching architectures', function () {
    var cases = [['/l', 'concat-parallel-utils'], ['/Z', 'concat-y-utils'], ['/y', 'concat-y-bad-conn'] // disconnected graph
    ];
    cases.forEach(pair => it('should validate ' + pair[1], run.bind(this, pair[0], pair[1], true)));
  });
  describe('mismatching architectures', function () {
    var cases = [['/l', 'concat-y-utils'], ['/y', 'concat-parallel-utils'], ['/s', 'concat-y-utils']];
    cases.forEach(pair => it('should NOT validate ' + pair[1], run.bind(this, pair[0], pair[1], false)));
  });
  describe('ignore option', function () {
    it('should ignore attributes as specified', function (done) {
      core.loadByPath(rootNode, '/Z').then(node => {
        return core.loadChildren(node);
      }).then(children => {
