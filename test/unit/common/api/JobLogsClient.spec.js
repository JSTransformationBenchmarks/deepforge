...
describe('JobLogsClient', function () {
  ...
  describe('appendTo', function () {
    before(function (done) {
      logClient.appendTo(jobId, firstLog).then(() => done()).catch(err => done(err));
    });
    ...
    describe('getLog', function () {
      ...
      it('should append additional logs to the file', function (done) {
        ...
        logClient.appendTo(jobId, secondLog).then(() => logClient.getLog(jobId)).then(log => {
  ...
  describe('delete', function () {
    ...
    it('should delete the file from job-logs directory', function (done) {
      logClient.deleteLog(delJobId).then(() => logClient.getLog(delJobId)).then(log => {
        expect(log).to.equal('');
        done();
      });
    });
  ...
  describe('migration', function () {
    ...
    before(function (done) {
      ...
      client.appendTo(jId, logs).then(() => client.fork(newBranch)).catch(err => done(err)).then(() => done());
    });
    ...
    describe('new logs', function () {
      ...
      it('should write new logs', function (done) {
        client.appendTo(jId, 'moreStuff').then(() => client.getLog(jId)).then(log => {
  ...
  describe('metadata', function () {
    ...
    it('should store the lineCount, cmdCount', function (done) {
      logClient.getMetadata(jobId).then(metadata => {
