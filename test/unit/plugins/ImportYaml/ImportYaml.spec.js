...
describe('ImportYaml', function () {
  ...
  before(function (done) {
    testFixture.clearDBAndGetGMEAuth(gmeConfig, projectName).then(function (gmeAuth_) {
      gmeAuth = gmeAuth_; // This uses in memory storage. Use testFixture.getMongoStorage to persist test to database.
  ...
  var runTest = async function (name, done) {
    var manager = new PluginCliManager(null, logger, gmeConfig),
        pluginConfig = {},
        context = {
      namespace: 'nn',
      ...
      activeNode: ''
        ...
        data = await fs_readFilePromise(path.join(YAML_DIR, name), 'utf8'),
        ymlFile = path.join(YAML_DIR, name.replace(/lua$/, 'yml')),
        yml = await fs_readFilePromise(ymlFile, 'utf8'),
  ...
  describe('run test cases', function () {
    ...
    .filter(name => path.extname(name) === '.yml'); // one test for each test name
