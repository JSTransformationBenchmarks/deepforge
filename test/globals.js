...
var testFixture = require('webgme/test/_globals'),
    ...
    WEBGME_CONFIG_PATH = `${__dirname}/../config`; // This flag will make sure the config.test.js is being used
...
testFixture.mkdir = async function (dir) {
  ...
  while (i++ < dirs.length) {
    ...
    if (!exists.sync(shortDir)) {
      await fs_mkdirPromise(shortDir);
    }
  }
};
